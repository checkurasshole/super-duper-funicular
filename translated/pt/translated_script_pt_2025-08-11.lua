...
local SettingsTab = Window:AddTab({ Title = "Definições", Icon = "settings" })
...
local function createPerformanceOverlay()
    ...
    title.Text = "RASTREADOR DE SESSÕES"
    ...
    toggleButton.Text = "Alternar sobreposição"
    ...
    fluentToggleButton.Text = "Ocultar IU"
...
local AutoBuySpinsToggle = MainTab:AddToggle("AutoBuySpins", {
    Title = "Rodadas de Compra Automática",
...
local AutoMachineToggle = MainTab:AddToggle("AutoMachine", {
    Title = "Desbloquear máquina automat...",
...
SettingsTab:AddButton({
    Title = "Redefinir estatísticas da s...",
    Callback = function()
        _G.planeconfig.beforecash = LocalPlayer.leaderstats.Cash.Value
        _G.planeconfig.sessionstart = tick()
        ...
        Fluent:Notify({
            Title = "Redefinição de sessão",
            Content = "O rastreamento de dinheiro foi redefinido!",
            Duration = 3
        })
    end
})
...
Fluent:Notify({
    ...
    Content = "Todos os recursos prontos para uso! W para VateQ",
    Duration = 5
})
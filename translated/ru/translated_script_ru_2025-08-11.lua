...
local MainTab = Window:AddTab({ Title = "Главное", Icon = "zap" })
local SettingsTab = Window:AddTab({ Title = "Параметры", Icon = "settings" })
...
local function createPerformanceOverlay()
    ...
    performanceGui.Name = "Оверсия результатов работы"
    ...
    toggleButton.Text = "Накладка@ info: whatsthis"
    ...
    fluentToggleButton.Text = "Скрыть UI"
...
local SuperFarmerToggle = MainTab:AddToggle("SuperFarmer", {
    Title = "Бесконечные денежные средства",
...
local AutoBuySpinsToggle = MainTab:AddToggle("AutoBuySpins", {
    Title = "Автоматически покупать скру...",
...
local AutoMachineToggle = MainTab:AddToggle("AutoMachine", {
    Title = "Автозаблокировать машину",
...
SettingsTab:AddButton({
    Title = "Перезагрузить записи сеансов",
    Callback = function()
        _G.planeconfig.beforecash = LocalPlayer.leaderstats.Cash.Value
        _G.planeconfig.sessionstart = tick()
        ...
        Fluent:Notify({
            Title = "Перезагрузка сеанса",
            Content = "Отслеживание наличных средств перезагружено!",
            Duration = 3
        })
    end
})
...
Fluent:Notify({
    Title = "Скрипт заряжен!",
    Content = "Все элементы готовы к использованию! W-VateQ",
    Duration = 5
})